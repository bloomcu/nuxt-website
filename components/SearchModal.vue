<template>
  <Modal uuid="search" title="Search">
    ...
    <div class="search-input search-input--icon-left">
      ...
      <input v-model="query" id="search-input" ref="input" class="search-input__input form-control" type="search" name="search-input" placeholder="What can we help you find?" autocomplete="off" aria-label="Search">

      <button v-if="query" @click="query = ''" class="search-input__btn search-input__clear" aria-hidden="true">
        <svg class="icon" viewBox="0 0 24 24"><title>Remove</title><g stroke-linecap="round" stroke-width="2" fill="none" stroke="currentColor" stroke-linejoin="round" class="nc-icon-wrapper"><line x1="19" y1="5" x2="5" y2="19"></line> <line x1="19" y1="19" x2="5" y2="5"></line></g></svg>
      </button>
    </div>
...
export default {
  ...
  setup(props, { root }) {
    ...
    const client = new MeiliSearch({
      host: root.$config.MEILISEARCH_HOST,
      apiKey: root.$config.MEILISEARCH_KEY,
    })

    // Handle focus on input when opened
    ...
    // Handle queries
    const query = ref('')
    const queryDebounced = useDebouncedRef('', 1000)
    const results = ref([])
    ...
    // Search from selected item in search history
